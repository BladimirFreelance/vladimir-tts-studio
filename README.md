...
## Windows Training GUI

Отдельный Windows GUI для запуска/остановки обучения без изменения существующего CLI и studio UI.

Запуск:

```powershell
.\.venv\Scripts\python.exe tools\win_train_gui.py
```

Что делает GUI:
- Режимы Scratch/Resume для `python -m app.main train ...`
- Показывает checkpoints и epoch/global_step
- Считает `Target max_epochs = resume_epoch + Add epochs`
- Логи пишет в `data/projects/<project>/runs/train_console.log` и читает их в окне GUI
- Процесс обучения запускается отдельно от GUI (закрытие GUI не блокирует stdout/stderr обучения)
