...
def _load_onnx_config(model_path: Path) -> dict:
...
def _prepare_input(model_path: Path, text: str, mode: str) -> str:
    if mode == "text":
        return text
    ...
    config = _load_onnx_config(model_path)
...
def synth_with_piper(model_path: Path, text: str, out_path: Path, mode: str = "text") -> None:
    ...
    input_text = _prepare_input(model_path, text, mode)
